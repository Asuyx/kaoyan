...
对于元素次序不重要的向量结构，在执行删除时，可以将被删除的元素移动到向量末尾再删除，可以将删除操作的时间复杂度从$\Theta(n-r)$​降低为$O(1)$​，这是一个常用的技巧。在后面的章节中还会再次出现。

### 拓展：静态列表删除的安全性问题

在动态列表中，“位置”使用指向节点的指针描述的。删除一个节点时，如果仍有指向它的指针，那么这个指针会变成**野指针**（wild pointer），这是没问题的。

而在静态列表中，“位置”是用元素在向量`V`中的秩来描述的。但使用上述方案的情况下，删除一个节点时，如果仍有指向它的秩，那么这个指针会错误地指向一个其他的元素（即移动过来的`V[last]`）；另一方面，如果有原先指向`V[last]`的秩，那么删除一个节点时，这个秩会指向一个不存在的位置（如果之后`V`又加入了新的元素，则同样会指向一个错误的元素）。
...
向量和列表作为线性表，存储了线性（一维）的数据。当一维来到二维（**矩阵**，matrix）甚至更高维（**张量**，tensor），存储的方式在实质上仍然是线性的，因为计算机的内存地址是线性的一个数值（标量），而不是二维或高维的一个坐标（矢量）。

> 在硬件上，存储器的单元可能是二维甚至三维的，但您将在《操作系统》中了解到，程序所使用的内存地址总是虚拟地址，和硬件并不能够形成严格对应。此外，因为CPU处理数据的时序是线性的，所以即使是硬件上的高维数据，遍历时也必须按线性的方式逐个交由CPU处理。

> 这一节在清华大学《数据结构》里没有，因为矩阵的相关内容被放到了《数值分析》那门课里。

对于一般的矩阵`A[0:m,0:n]`，它的存储需要`mn`个数据单元。如果一行一行地存储（同一行的数据，内存地址是连续的），就称为**按行优先**；如果一列一列地存储，就称为**按列优先**。在C语言和C++中的二维数组`A[0:m][0:n]`是按行优先存储的，而MATLAB等语言中二维数组是按列优先存储的。

> 按行优先的意思是，`A[0:m][0:n]`会被展开成：
>
> `A[0][0],A[0][1],...,A[0][n-1],A[1][0],A[1][1],...,A[1][n-1],...`这样的一个序列，这个序列存储在内存中的方式仍然是也只能是线性的。

所谓矩阵的压缩存储，就是对矩阵`A[0:m,0:n]`来说，如果先验地知道矩阵所满足的一些性质，那么矩阵并没有`mn`个自由度，也就不需要用`mn`个数据单元去存储它了。比如三角矩阵和对称矩阵，都只有大约一半的元素需要存储。

> 这个具体公式没有必要记忆，现场算一下就行。

另一种情况就是所谓的**稀疏矩阵**。

稀疏矩阵的意思是其中的绝大多数元素是0（虽然不知道哪些元素是0），比如每行只有一个非0元素。这种矩阵在人工智能领域非常常见。存储稀疏矩阵的一个方法是**三元组**法，就是用`(行号, 列号, 值)`的三元组数组存储所有的非0元素。三元组数组的压缩效率非常高，但是要“循秩”访问`A[i][j]`的时候就必须遍历整个数组查找，访问效率非常低（类似于列表）。

介于三元组数组和二维数组之间，还有一种存储方式是**十字链表**，也就是在行和列两个方向上都组织成链表形式。下图展示了单向（基于单链的）十字链表的组成结构，双向十字链表增加了两个方向的`pred`指针、尾节点`tail`等，与之大同小异。

![十字链表](..\..\pic\ds\十字链表.png)

其中，**行头节点**和**列头节点**通常用两个数组（或向量）保存，使得能在$O(1)$的时间内定位到某一行（或列）的头节点。当矩阵是方阵的时候行和列数量相同，这时也可以将其整合为一个节点。

十字链表因为要存储2或4个方向（取决于单链还是双链）的指针以及每行/列的头节点，所以压缩率不如三元组，不过相比于$\Theta(mn)$的二维数组来说已经压缩率高了很多。另一方面，在访问`A[i][j]`的时候，十字链表可以直接定位到相应的行或列，从而大大降低了查找的时间。因此，它相当于在压缩率和访问效率上，形成了对二维数组和三元组的折中。

十字链表这种数据结构对于考研复习来说过于复杂，通常只需要知道它的存在即可，基本不可能会出现建立在十字链表上的分析题。
...
作为分块表的整体，它包括一个一级表（**索引表**）、一个二级表（**数据表**）。其中，索引表的每个节点需要记录索引指针所指向的二级表节点，以及它指向的节点在表中的秩。这个两级结构可以用下图描述。

![分块结构](..\..\pic\ds\分块结构.png)
...
> 如果要追求最高的效率，那么需要判断`index`在前半段还是后半段，决定是从前向后还是从后向前检索。在**算法2.18A**中简单地都实现成了从前向后。
...
如果使用之前设计的`ListList`框架，就会发现，循秩的插入（删除）只需要$\Theta(\sqrt n)$​的时间：因为在循秩访问的过程中，已经可以确定被插入（删除）的节点在哪两个索引之间。所以循秩的插入（删除）只需要三步：循秩访问—在`L2`上做循位置插入（删除）—更新`L1`中的索引节点。

> 在分块表上直接做循位置插入（删除），则需要平均$\Theta(n)$​的时间。
>
> 因为对于循位置的插入（删除），无法直接定位被插入（删除）的位置在哪两个索引节点指向的位置之间，需要平均$\Theta(n)$的时间遍历整个表，才能找到目标位置。所以，分块表应当避免使用循位置的插入（删除）。
...
> 上面的实现在空表插入第一个节点的时候会发生错误（第一行的`L1[0]`没有定义）。请您对**算法2.19A**进行修改，以规避该错误。一个修改的示例见配套代码。

删除的时候会遇到一个问题：`L1`中的某个索引节点，恰好指向即将被删除的数据节点。那么在删除数据节点之后，这个索引会变成野指针，必须要加以处理。
...
>
> 在本节的后续内容中，将使用“索引向后`1`个节点”的策略（如果导致两个索引重合，就删掉一个），参见配套代码。这一策略能保证相邻索引节点指向的秩的距离不会发生突变。

### 拓展：重构索引表

在分块表中，一级表（索引表）需要满足两个基本性质：

* 每块数据表的长度是$O(\sqrt n)$。即相邻索引节点指向的秩，其距离是$O(\sqrt n)$。
* 索引表的长度是$O(\sqrt n)$。
...
设$k_1,k_2>1$是两个预先设定的比例阈值（为避免重构过于频繁，应取$k_1,k_2\ge 2$），当相邻索引节点指向的秩距离超过$k_1\sqrt n$的时候，对索引表进行**局部重构**（只需要处理这对相邻索引节点的问题）；当索引表的长度超过$k_2\sqrt n$的时候，对索引表进行**整体重构**（需要压缩整体的索引表）。

**局部重构**：设索引节点`p1`和`p2`之间的距离超过了阈值。那么，需要对`p1`和`p2`指向的节点之间的二级表进行遍历，每隔$\sqrt n$​，插入一个指向它的索引节点。

```c++
// 算法2.20A
// p1 和 p2 = p1->succ 之间的距离超出了阈值
template <typename T>
...
> 对考研而言，只需要知道循秩操作<u>确实</u>能在$O(\sqrt n)$​时间内完成就可以了。
