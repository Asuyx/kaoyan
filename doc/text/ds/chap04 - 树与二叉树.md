...
默认`visit`函数的时间、空间复杂度都是$O(1)$的，那么您可以利用递归法，轻松证明三种遍历的时间复杂度都是$\Theta(n)$，空间复杂度都是$\Theta(h)$，其中`h`二叉树的高度。在退化成链的情况下，空间复杂度会取到最坏的$O(n)$；而在完全二叉树的情况下，空间复杂度可以取到最好的$\Omega(\log n)$。
...
上图展示了表达式经由栈向二叉树的转换过程。反之，通过黄色的虚线边，表达式树也可以转换回三种表达式（表达式树的中序序列，和中缀表达式并不严格对应，因为其中不会含有表示运算顺序的括号）。
