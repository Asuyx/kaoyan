...
和**算法4.3AS**相比，**算法4.3ASS**在时间和空间上都有着一定程度的优化。

> 当然，这个优化程度非常小，而且破坏了**算法4.3AS**的形式美感。从记忆的角度上，笔者还是认为**算法4.3AS**更方便记忆。引入**算法4.3ASS**，是希望从先序遍历（使用栈）自然地过渡到中序遍历（使用栈），帮助您理解下一小节的内容。

### 无递归的中序遍历

当讨论到中序遍历的时候，就不再是尾递归了。这意味着不方便使用**算法4.3AS**的消除尾递归的思路，但**算法4.3ASS**的思路仍然有用。

对于先序遍历，一路向左下的过程中，会访问沿途经过的节点，然后在回溯时陆续访问右姊妹；而在中序遍历里，同样是一路向左下，但并不会访问沿途经过的节点，而是从最左下的那个节点出发，在回溯时陆续访问途径上的节点，然后再转入右姊妹。

所以，只需要在**算法4.3ASS**上进行比较小程度的修改。

1. 在沿着左子代到底的过程中不访问经过的节点，而是将这些节点暂存到栈里。
2. 在回溯的过程中，再访问从栈里弹出的节点，然后转向其<u>右子代</u>。

```c++
// 算法4.3BSS - 中序遍历（使用栈，利用二叉树结构）
template <typename T>
...
> 迭代版本的三种递归，先序遍历（**算法4.3AS**）和下一小节介绍的、基于对称性由先序遍历推出的后序遍历（**算法4.3CS**），都是直接或间接地由上一章所述的尾递归消除技术得到的，具有对称和简洁的美感，从而非常方便记忆。而**算法4.3ASS**、**算法4.3BSS**以及下一节的**算法4.3CSS**，由于利用了对二叉树结构及三种遍历性质的分析，算法的形式美感被破坏了，因此记忆上有一定的难度。笔者希望您理解这些算法的推导过程，知道推导它们的切入点，如有必要，在考场上现场推导即可。

### 二叉树上的线性递归

在此前的小节里，曾经讨论过利用先序序列和中序序列重建二叉树的问题。现在已经对先序遍历和中序遍历有更深刻的认识，可以回到这个问题上来了。

在之前使用的**算法4.4A**，时间复杂度是$\Theta(nh)=O(n^2)$​​的，这并不符合预期。既然二叉树的遍历（<u>半线性结构线性化</u>）是$\Theta(n)$​的，那么根据对称原则，在预期上，二叉树的重建（<u>线性序列半线性化</u>）也应该是$\Theta(n)$。

前面已经讨论过，**算法4.4A**本身并没有做重复计算；那么它在时间复杂度上有缺陷的原因，就在于它没有充分利用先序序列和中序序列的性质，损失了一些先验信息。而分析**算法4.3ASS**和**算法4.3BSS**的思路，可以对先序序列和中序序列的性质有更加深刻的理解，找到先验信息究竟损失在哪里。

![先序遍历和中序遍历](../../pic/ds/先序遍历和中序遍历.png)

图中的红色虚线箭头表示了先序遍历和中序遍历的次序。您可以对照**算法4.3ASS**和**算法4.3BSS**，会发现，这两个非递归的算法，并不是由经典的递归算法**算法4.3A**和**算法4.3B**引申出的，而采用的是上图所示的递归方法。

> 您可以根据上图所示的递归思路，将原始的树上的遍历，递归到`T[0:n]`上的分别遍历，实现相应的递归算法。这才是**算法4.3ASS**和**算法4.3BSS**本质上所基于的递归算法。

这个递归思路和常规的（**算法4.3A**和**算法4.3B**）递归思路，在设计思想上的区别是：它隐含着<u>半线性结构的线性化</u>。

> 在常规的递归思路下，一个二叉树会被递归到它的左右子树。这个思路利用了二叉树的天然的半线性性质（下称**半线性递归**），除树高外，左右子树和原树具有结构相似性，左右子树之间具有对称性；这一良好的性质，使得以这个递归思路可以很方便地使用N、L、R三者的组合来设计树上的算法，且算法性能分析也较为简便。
>
> 而上图所示的递归思路，构造了一个二叉树的基本骨架。这个基本骨架就是从树根出发，不断向左下延伸，直到没有左子代的节点为止的这一条路径；称为**最左侧通路**。最左侧通路是一条链，而递归发生在最左侧通路挂载的右子树上。由于最左侧通路是链状的而非树状的，所以这种递归思路，本质上蕴含了<u>半线性结构的线性化</u>（下称**线性递归**）。
>
> > 半线性递归中，N、L、R三者的次序是可以任意组合的，比如NLRN这种“先+后递归”也经常用到，它是一种树上做递归的思路，而不是具体的某种递归。
> >
> > 类似地，线性递归也是一种比较抽象的思路，上图所示的最左侧通路骨架，只是线性递归的一种形式，它比较适合用来处理先序序列和中序序列的问题而已。
>
...
>
> 无论是先序序列还是中序序列，其本质上也是一种<u>半线性结构的线性化</u>：将二叉树上的节点通过访问的次序，转换成节点序列。而线性递归所采用的骨架结构，则是先序序列和中序序列的一种结构上的统一。因此，线性递归的这个结构，可以很好地应用于先序序列和中序序列的问题。
>
> 线性递归的思路，在遍历序列这种线性问题上是有价值的，但它破坏了二叉树的天然半线性性质，子树线性表中的各个元素`T[0:n]`不对称。这会给算法的理解、分析和设计带来额外的复杂性。

### 使用线性递归完成二叉树的重建

线性递归的思路，已经在**算法4.3ASS**和**算法4.3BSS**中，展现了<u>半线性结构线性化</u>上的作用。这一小节回到上一小节最初提到的话题上来。利用先序序列和中序序列重建二叉树，这是一个<u>线性结构半线性化</u>的过程。那么，<u>在搭建半线性化结构时，可以采用线性的骨架作为基础</u>。这就又可以利用到线性递归的思路了。

> **算法4.4A**所损失的先验信息，正是线性递归所反映出的、先序序列和中序序列的内在统一性：它们可以被统一为最左侧通路序列、和它挂载的右子树序列的不同排列。
...
> 由于`lc`和`parent`指针的存在，最左侧通路`A[0:n]`实质上可以被当做一个双向链表。而在上述的重建算法中只包含循位置访问，所以这个双向链表已经够用，不需要额外花费空间去存储这个线性表。
>
> 当然，如果是子代表示法，没有`parent`指针，那么为了回溯，就必须要使用一个栈存储途径的节点，就像**算法4.3BSS**一样，请您自己对下面的算法进行修改以完成这一功能。
...
BinaryTree<T> rebuildBinaryTree2(const Vector<T>& preSeq, const Vector<T>& inSeq) {
    ...
    buildSubtree = [&](bool end, const T& endPoint) -> BTNode<T>* {
        ...
        for (++i, ++j; j < n && (end || inSeq[j] != endPoint); last = last->parent, ++i, ++j) {
            (last->rc = buildSubtree(preSeq[i]))->parent = last; // 利用已知边界构造右子树
        }
        while (last->parent != nullptr) {              // 即使达到了边界
...
> 上述代码中有一个地方破坏了美感：对于递归下去的子树，都是用终止符（如`T[n-1]`的终止符是`A[n-2]`）来界定的；但对于最外层的整棵树，是用序列终止界定的，没有终止符。这导致在`buildSubTree`函数中不能只接受一个参数（终止符`endPoint`，需要额外增加一个“是否有终止符”的参数`end`）。

代码比较长，但没必要记忆，它的思路是非常清晰的。
