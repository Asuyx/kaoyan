...
这一小节从扩展Catalan数的角度出发，讨论如何生成一个随机的栈操作序列。

考虑包含$p$个`push`和$q$个`pop`的序列，其中$0\le p\le q $。如果在这个序列前添加$q-p$个`push`可以得到合法的栈操作序列，则下文称其为$p+q$的**后缀操作序列**（显然，一个合法操作序列的任一后缀都是后缀操作序列）。设$p+q$的后缀操作序列有$C_{p,q}$个，则可以得到$C_{n,n}=\mathrm{Catalan}(n)$。

直接对栈的操作序列做递归比较困难。而定义了后缀栈操作序列之后，就可以研究生成过程中的中间结果。

> 总体目标是生成$n+n$的后缀操作序列。
>
> 如果从前到后生成，在某一步处已经生成了$n-p$个`push`和$n-q$个`pop`，那么剩余部分是一个$p+q$的后缀操作序列，有$C_{p,q}$种等概率的可能性。从而，这一步生成`push`的概率是$C_{p-1,q}/C_{p,q}$，生成`pop`的概率是$C_{p,q-1}/C_{p,q}$。

上述结论表明，只要计算出$C_{p,q}$的通项公式，就可以从前向后随机地逐个生成操作序列上的操作。

> 使用上一小节介绍的、基于一一映射的计数方法，容易证明：
> $$
...
> $$
> 这意味着不需要实际计算组合数，就可以得到每次生成`push`和`pop`的概率。

那么，可以得到下列时间复杂度为$\Theta(n)$的算法。
...
>
> 从这个角度，容易推导出算法复杂度是$\Theta(C_m^n)$​的，这个复杂度相当高。可以通过将递归改写成递推来降低复杂度到$O(mn)$​。用递推消除递归不属于《数据结构》的考试范围，将在《算法设计》中简要介绍。
...
> 循环队列并没有标准的实现形式。有些教材中，会将循环队列的<u>实际容量</u>设置为`m+1`，使得在队列满（规模为`m`）时，`rear`和`front`仍然有`1`的差距，从而和队列空相区别。
>
> 笔者认为这不是一种好的方法。因为对于循环队列的每个元素非常大，而队列容量比较小（比如说，队列的每个元素需要包含一个I/O缓冲区）时，“浪费一个位置”是非常奢侈的行为。相比之下，一个`empty`指标总是可以用一个变量（在硬件中，一个寄存器）来表示。
